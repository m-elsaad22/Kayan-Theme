...
if ( ! function_exists( 'kayan_homepage_build_nav_links_html' ) ) {
	function kayan_homepage_build_nav_links_html( $menu_location = 'main-menu', $css_class = 'menu' ) {
		...
		$fallback = array(
			array( '#services', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_services' ) : 'الخدمات' ),
			array( '#areas', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_cities' ) : 'المدن' ),
			array( '#projects', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_projects' ) : 'المشاريع' ),
			array( '#blog', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_blog' ) : 'المدونة' ),
			array( '#why', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_about' ) : 'من نحن' ),
		);
	...
	function kayan_homepage_build_mobile_nav_html() {
		...
		if ( ! empty( $items ) && is_array( $items ) ) {
			foreach ( $items as $item ) {
				...
				$html .= '<a href="' . esc_url( $item->url ) . '" onclick="toggleMob(false)">' . esc_html( $item->title ) . '</a>';
			}
		} else {
			$fallback = array(
				array( '#services', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_services' ) : 'الخدمات' ),
				...
				array( '#blog', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_blog' ) : 'المدونة' ),
				array( '#why', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_about' ) : 'من نحن' ),
				array( '#faq', function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'nav_faq' ) : 'الأسئلة الشائعة' ),
			);
			foreach ( $fallback as $link ) {
		...
		$html .= '<a href="' . esc_url( kayan_homepage_get_whatsapp_url() ) . '" class="btn btn-wa" onclick="toggleMob(false)"><i class="fab fa-whatsapp"></i> ' . esc_html( function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'btn_whatsapp_full' ) : 'واتساب' ) . '</a>';
		if ( kayan_homepage_get_phone_raw() !== '' && ( ! function_exists( 'kayan_ui_show_call_button' ) || kayan_ui_show_call_button() ) ) {
			$html .= '<a href="' . esc_url( kayan_homepage_get_tel_url() ) . '" class="btn btn-call" onclick="toggleMob(false)"><i class="fas fa-phone"></i> ' . esc_html( function_exists( 'kayan_i18n_t' ) ? kayan_i18n_t( 'btn_call' ) : 'اتصل' ) . '</a>';
		}
		return $html;
	}
}
...
if ( ! function_exists( 'kayan_homepage_build_footer_social_html' ) ) {
	function kayan_homepage_build_footer_social_html() {
		...
		$wa = kayan_homepage_get_whatsapp_url();
		if ( $wa !== '#' ) {
	...
	function kayan_homepage_build_menu_column_html( $menu_id, $title, $fallback_links = array() ) {
		...
		if ( ! empty( $items ) && is_array( $items ) ) {
			foreach ( $items as $item ) {
				...
				$html .= '<li><a href="' . esc_url( $item->url ) . '"><i class="fas fa-chevron-left"></i> ' . esc_html( $item->title ) . '</a></li>';
			}
		} elseif ( ! empty( $fallback_links ) ) {
			foreach ( $fallback_links as $link ) {
				$html .= '<li><a href="' . esc_url( $link['url'] ) . '"><i class="fas fa-chevron-left"></i> ' . esc_html( $link['label'] ) . '</a></li>';
			}
		}
		$html .= '</ul></div>';
		return $html;
	...
	function kayan_homepage_build_services_grid_html() {
		...
		if ( is_array( $custom ) && ! empty( $custom ) ) {
			...
			foreach ( $custom as $card ) {
				...
				$url   = ! empty( $card['url'] ) ? $card['url'] : '#contact';
