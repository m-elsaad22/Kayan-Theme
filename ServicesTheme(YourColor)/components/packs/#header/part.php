...
# LOGO EDITS .
	...
	$logo__mode = ( isset( $logo__data['logo__mode'] ) && is_string( $logo__data['logo__mode'] ) ) ? $logo__data['logo__mode'] : '';
	if( empty( $logo__data ) || empty( $logo__mode ) || ! isset( $logo__data[ $logo__mode ] ) ) {
		$logo__data = array(
			'logo__mode' => 'Text',
			'Text'       => array(
				'logo_Text'       => 'YOUR{%COLOR%}',
				'secondary_color' => ( ! empty( $site_color ) ) ? $site_color : '#1269eb',
				'header__alt'     => get_bloginfo( 'name' ),
			),
		);
	}
