<?
function kayan_breadcrumb_finish_output( $html ) {
	if ( function_exists( 'kayan_seo_uses_modern_schema' ) && kayan_seo_uses_modern_schema() ) {
		$html = preg_replace( '/\s*itemscope(?:\s+itemtype="[^"]*")?/', '', $html );
		$html = preg_replace( '/\s*itemprop="[^"]*"/', '', $html );
		$html = preg_replace( '/<meta\s+itemprop="position"[^>]*\/?>\s*/', '', $html );
	} else {
		$html = str_replace( 'http://schema.org', 'https://schema.org', $html );
	}
	echo $html;
}
...
function Breadcrumb() {
	ob_start();
	$position = 1;
	echo '<ol itemscope itemtype="https://schema.org/BreadcrumbList" class="BreadcrumbsFilters">';
	...
	echo '</ol>';
	kayan_breadcrumb_finish_output( ob_get_clean() );
}
