...
$target = false;
$hide__price__button = yc_get_option('hide__price__button');
if( empty( $hide__price__button ) ){
	$price__button__text = get_post_meta( $post->ID,'btn_title',true );
	if( empty( $price__button__text ) ) $price__button__text = yc_get_option('price__button__text');

	if( !empty( $price__button__text ) ) $ButtonText = $price__button__text;
	$price__option_data = yc_get_option('price__option_data');
	if( !empty( $price__option_data ) && isset( $price__option_data['price__mode'] ) && isset( $price__option_data[ $price__option_data['price__mode'] ] ) ){
		...
		if( $price__option_data['price__mode'] == 'manual' ){
			...
			}else if( !empty( yc_get_option('whatsapp_number') ) ){
			...
			}else if( !empty( yc_get_option('phonenumber') ) ){
	...
	if( !isset( $PermaLink ) || isset( $PermaLink ) && empty( $PermaLink ) ){
		$contact__us__page = yc_get_option('contact__us__page');
...
#
$currency__shows = yc_get_option('currency__shows');
...
# PRICE VALUE .
	...
	if( empty( $price_icon ) ) $price_icon = yc_get_option('currency');
