...
# PAGE CONTENT.
	...
	# POST TERMS.
		...
		# POPOVER EDITS
		    $PopOver__Attr = '';
			$hide__post__popover = yc_get_option('hide__post__popover');
			if( empty( $hide__post__popover ) ) {
				...
				if( empty( $meta_hide_popover ) ){
					...
					if( $found__popover == false ){
						$post__popover__data = yc_get_option('post__popover__data');
						$post__popover__data = ( ( is_array( $post__popover__data ) ) ) ? $post__popover__data : array();
						if( !empty( $post__popover__data ) && isset( $post__popover__data['popover_call_title'] ) && !empty( $post__popover__data['popover_call_title'] ) ){
							$found__popover = true;
							$post__popover__data['whatsapp_number'] = $whatsapp_number;
		...
		# SINGLE OPTIONS .
			$show_comments = ( ( wp_is_mobile() ) ) ? yc_get_option('disable_mobile_comments') : yc_get_option('disable_comments');
			$hide__thumbnail__single = yc_get_option('hide__thumbnail__single');
			$hide__next_prev_post__single = yc_get_option('hide__next_prev_post__single');
			$hide__post__references = yc_get_option('hide__post__references');
			$hide__post__faqs = yc_get_option('hide__post__faqs');
			$hide__post__shares = yc_get_option('hide__post__shares');
			$hide__feedback__rating = yc_get_option('hide__feedback__rating');
			$hide__post__tags = yc_get_option('hide__post__tags');
			$hide__related__section = yc_get_option('hide__related__section');

			$hide__post__boxx = yc_get_option('hide__post__boxx');
			$hide__post__category = yc_get_option('hide__post__category');
			$hide__post__author = yc_get_option('hide__post__author');
			$hide__post__date = yc_get_option('hide__post__date');

			# SINGLE SIDE BAR .
			$hide__sidebar__single = yc_get_option('hide__sidebar__single');
			if( empty( $hide__sidebar__single ) ){

				$widgets_single__meta = ( is_array( yc_get_option( 'widgets_single__meta' ) ) ) ? yc_get_option( 'widgets_single__meta' ) : array();
		...
		echo '<div class="-singular-pages-container"'.$PopOver__Attr.'>';
			echo '<div class="container">';
				...
				echo '<div class="-Yc-single-main -YC-singleType-'.$post->post_type.'">';
					...
					$hide__sidebar__service_request_single = yc_get_option('hide__sidebar__service_request_single');
